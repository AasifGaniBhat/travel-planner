...
import TravelPlannerCards from "../components/travelplanner-cards/indes";
...
function Home() {
  ...
  const tabs = ["Kashmir packages", "Outside Kashmir packages", "Bihar packages"];
...
<Tabs data={tabs} setTab={setSelectedTab} />
              ...
              <div className={classes.events_container}>
                {selectedTab === 0
                  ? events.filter(pack => pack.kashmir === true)?.slice(0,6)?.map((event, index) => {
                      return (
                        <EventItem
                          event={event}
                          index={index}
                  ...
                  :selectedTab === 1? events.filter(pack => pack.kashmir === false)?.slice(0,6)?.map((event, index) => {
                    return (
                      <EventItem
                        event={event}
                        index={index}
